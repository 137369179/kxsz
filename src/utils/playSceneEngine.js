  ...
  renderEducationalWood() {
    this.mount.innerHTML = `
      <div class="relative w-full h-full bg-gradient-to-b from-sky-300 to-emerald-200 overflow-hidden select-none touch-none flex flex-col items-center cursor-crosshair">
        ...
        <!--  -->
        <svg id="slash-trail-container" class="absolute inset-0 w-full h-full pointer-events-none z-50">
           <polyline id="slash-trail" fill="none" stroke="rgba(255,255,255,0.8)" stroke-width="8" stroke-linecap="round" stroke-linejoin="round" filter="drop-shadow(0 0 8px rgba(255,255,255,1))" />
        </svg>
    ...
    const trail = this.mount.querySelector("#slash-trail");
    const skeleton = this.mount.querySelector("#tree-skeleton");
    let cleared = 0, isSlashing = false, points = [];

    // Slash Trail Logic
    this.onWindow("pointerdown", (e) => { isSlashing = true; points = [[e.clientX, e.clientY]]; });
    this.onWindow("pointermove", (e) => {
